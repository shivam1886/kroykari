...
class HomeController extends Controller
{
    ...
    public function adStore(Request $request){
       ...
       $rules = [
        ...
        $dynamicFieldData = $request->except('_token','category','sub_category','title','description','price','city','local_area','image','is_deliver_to_buyer','is_negotiable','email','phone','lat','lng','phone_number','phone_numbers',
      'is_deliver_to_buyer');
        ...
        $adData = [
          ...
          'phone_numbers'        =>  $input['phone_numbers'] ? serialize($input['phone_numbers']) :  serialize(array()),
    ...
    public function adUpdate(Request $request,$id){
       ...
       $rules = [
        ...
        try {
                  ...
                  if(isset($input['old']) && !empty($input['old']))
                        DB::table('ad_images')->where('ad_id',$id)->whereNotIn('id',$input['old'])->delete();
         ...
         if($request->hasFile('image')){
          ...
          $dynamicFieldData = $request->except('_token','category','sub_category','title','description','price','city','local_area','image','_method','old','is_deliver_to_buyer','is_negotiable','email','phone','lat','lng','is_hide_phone_number',
            ...
            if($adsData->toArray()){
                foreach($adsData as $key => $value){
                  ...
                  if($value->receiver_id == auth::id()){
                        ...
                        $tmp['last_seen'] = $value->sender->last_seen;
                        $tmp['is_online'] = '1';
                  }
                  if($value->sender_id == auth::id()){
                        $tmp['user_id']       = $value->receiver_id;
                        $tmp['last_seen'] = $value->receiver->last_seen;
                        $tmp['is_online'] = '1';
                  }
    ...
    public function getCategories(){
        return $data = Category::whereNull('parent_id')->where('is_active','1')->whereNull('deleted_at')->orderBy('title','asc')->get();
    }
