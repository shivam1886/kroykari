	...
	<section class="post-ad-form">
		<div class="container">
			<div class="left-form-details">
				<div class="heading">
					<h2>{{__('Sell an item or service')}} <span id="locationModalOpenBtn"><i class="fas fa-map-marker" style="color:#142a5b"></i>&nbsp;<small class="location-text"></small></span>&nbsp;<span id="categoryModalOpenBtn"><i class="fas fa-tags"  style="color:#142a5b"></i>&nbsp;<small class="category-text"></small></span></h2>
				 ...
				 <form action="{{route('ad.store')}}" method="post" id="create-ad-form" enctype="multipart/form-data">
					...
					<div class="body">
					   ...
					   <div class="photo-info">
						...
						<div class="add-btn">
							<button id="submit-form" type="submit">{{__('Add Post')}}</button>
						</div>
				    </div>
			    </form>
			</div><!--right-->
		    ...
		    <!-- The Modal -->
			<div class="modal fade custom-modal posting-allowance-modal" id="posting-safety-modal">
				<div class="modal-dialog modal-dialog-centered">
					<div class="modal-content">
					
						<!-- Modal Header -->
						<div class="modal-header">
							<h4 class="modal-title">{{__('Safety First!')}}</h4>
						...
						<div class="modal-body">
							<div class="text">
                               ...
                               <button class="yes-agree">{{__('Yes Agree')}}</button>
							</div>
						</div>                
					</div>
				</div>
		 ...
		 fields.map(function(field,index){
				...
				if(field.field_type == 'select'){
					let TextField  = '<div class="col-md-6 col-sm-6">';
						...
						TextField += 	 `<select name="${field.field_id}" class="form-control">`;
				...
				if(field.field_type == 'text'){
					let TextField  = '<div class="col-md-6 col-sm-6">';
						...
						TextField += 	  `<input type="text" name="${field.field_id}" value="" class="form-control"/>&nbsp;&nbsp;`;
						TextField +=  	'</div>';
						TextField += '</div>';
						$('.dynamic-fields').append(TextField);
				}
				...
				if(field.field_type == 'radio'){
					let TextField  = '<div class="col-md-6 col-sm-6">';
						...
						TextField += 	  `<label>${field.field_name}</label>`;
						field.options.map(function(option,index){
						TextField +=      `<input type="radio" name="${field.field_id}" value="${option.option}" /> ${option.label}&nbsp;&nbsp;`;
						});
						TextField +=  	'</div>';
						TextField += '</div>';
						$('.dynamic-fields').append(TextField);
				...
				if(field.field_type == 'checkbox'){
					let TextField  = '<div class="col-md-6 col-sm-6">';
						...
						TextField += 	  `<label>${field.field_name}</label>`;
						field.options.map(function(option,index){
						TextField +=      `<input type="checkbox" name="${field.field_id}[]" value="${option.option}" /> ${option.label}&nbsp;&nbsp;`;
						});
						TextField +=  	'</div>';
						TextField += '</div>';
						$('.dynamic-fields').append(TextField);
